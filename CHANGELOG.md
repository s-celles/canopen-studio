...
- FD over the compact binary wire format, in 80 bytes rather than 24. A classic frame is still exactly the 24 bytes it was, so a peer built before this reads it unchanged; an FD frame sets a flag bit that such a peer refuses on length rather than decoding truncated. A short FD frame is refused rather than zero-padded, because the missing bytes are payload.
- `CanFrame.is_fd`, `.bitrate_switch`, `.error_state_indicator` and `.dlc_code` on the Python extension, and `canopen_core.CanFrame(..., is_fd=True, bitrate_switch=True)`.
- **CAN FD's CRC-17 and CRC-21** (`crc17`, `crc21`, `crc_fd`, `fd_crc_width`), the payload turning over from one to the other above 16 bytes. Both are checked against the value the CRC catalogues publish for `"123456789"` — 0x04F03 and 0x0ED841 — because a round trip against our own encoder would pass just as happily with the wrong polynomial. The register seed is an argument rather than a constant: ISO 11898-1:2015 seeds it with a leading one and the 2012 Bosch version with zero, and controllers on either side of that do not interoperate.
- **CAN FD over the SLCAN serial transport**, the four letters `d`, `D`, `b` and `B` — FD standard and extended, without and with bitrate switch. The trap of that grammar is that the length digit is the **DLC code** rather than the byte count: `9` asks for twelve bytes and `F` for sixty-four, and no digit means nine. FD has no standard here, the firmwares disagreeing between `x`/`X` and a leading `0`/`1`; this follows `python-can`'s slcan driver, which follows the CANable 2.0 firmware — the adapter family the module already names. `x` is deliberately left alone, since in that dialect it spells a *classic* extended frame. A classic line whose digit exceeds 8 is still refused rather than reread as FD.
- **CAN FD in the PCAP-NG capture**, so Wireshark reads an FD bus. There is no separate link type for it: `LINKTYPE_CAN_SOCKETCAN` carries both, and a reader tells them apart by the record length — 16 bytes for classic, 72 for `struct canfd_frame` — together with `CANFD_FDF` in the flags byte. The two bytes behind those flags are written zero, because that byte was once reserved and a reader only trusts it when they are.

### Changed
- `CanFrame` is 88 bytes rather than 24, the payload buffer having grown to hold both formats in one type — the way `python-can`'s `Message` does — instead of splitting every consumer across an enum. What the benchmarks rest on is unchanged: no allocation, no garbage collector, `Copy` in and out of the ring buffer, and classic frames still cost 24 bytes on the wire.
- The VCD writer refuses an FD frame instead of drawing it wrong. The waveform is rebuilt with the classic bit rules, and FD switches bit rate mid-frame, stuffs differently and uses CRC-17 or CRC-21 in place of CRC-15, so the picture would match no analyser. It returns an error naming the reason. The bitstream decoder already refused a recessive r1, and still does.
