    ...
    def process_can_message(self, msg: can.Message) -> CanopenMessage:
        ...
        try:
            ...
            if is_ext:
                ...
                service = CanopenService.TPDO; pdo_num = 1; node_id = cid - 0x180; info = f"TPDO1 Node {node_id}"
            elif 0x200 <= cid <= 0x27F:
                service = CanopenService.RPDO; pdo_num = 1; node_id = cid - 0x200; info = f"RPDO1 Node {node_id}"
            elif 0x280 <= cid <= 0x2FF:
                service = CanopenService.TPDO; pdo_num = 2; node_id = cid - 0x280; info = f"TPDO2 Node {node_id}"
            elif 0x300 <= cid <= 0x37F:
                service = CanopenService.RPDO; pdo_num = 2; node_id = cid - 0x300; info = f"RPDO2 Node {node_id}"
            elif 0x380 <= cid <= 0x3FF:
                service = CanopenService.TPDO; pdo_num = 3; node_id = cid - 0x380; info = f"TPDO3 Node {node_id}"
            elif 0x400 <= cid <= 0x47F:
                service = CanopenService.RPDO; pdo_num = 3; node_id = cid - 0x400; info = f"RPDO3 Node {node_id}"
            elif 0x480 <= cid <= 0x4FF:
                service = CanopenService.TPDO; pdo_num = 4; node_id = cid - 0x480; info = f"TPDO4 Node {node_id}"
            elif 0x500 <= cid <= 0x57F:
                service = CanopenService.RPDO; pdo_num = 4; node_id = cid - 0x500; info = f"RPDO4 Node {node_id}"
            elif 0x580 <= cid <= 0x5FF:
                service = CanopenService.SDO_TX; node_id = cid - 0x580
                if len(data) >= 4:
                    idx = data[1] | (data[2] << 8)
                    sub = data[3]
                ...
                service = CanopenService.SDO_RX; node_id = cid - 0x600
                if len(data) >= 4:
                    idx = data[1] | (data[2] << 8)
                    sub = data[3]
