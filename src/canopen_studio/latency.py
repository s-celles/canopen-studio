...
class LatencyTracker:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            jitter_avg = (
                (sum(self.sync_jitter_history) / len(self.sync_jitter_history)) if self.sync_jitter_history else None
            )
            ...
            return {
                ...
                "sync_interval_last_ms": round(self.sync_interval_last_ms, 2)
                if self.sync_interval_last_ms is not None
                else None,
                "sync_jitter_last_ms": round(self.sync_jitter_last_ms, 2)
                if self.sync_jitter_last_ms is not None
                else None,
