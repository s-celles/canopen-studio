...
impl SdoAbortCode {
    pub fn description(&self) -> &'static str {
        match self {
            ...
            Self::GeneralInternalIncompatibility => {
            ...
            Self::DataCannotBeTransferred => {
                "Data cannot be transferred or stored to the application"
            }
            Self::DataCannotBeTransferredLocalControl => {
                "Data cannot be transferred or stored to the application because of local control"
            }
            Self::DataCannotBeTransferredDeviceState => {
                "Data cannot be transferred or stored to the application because of the present device state"
            }
            Self::ObjectDictionaryDynamicGenerationFails => {
...
pub fn parse_sdo_frame(frame: &CanFrame) -> Option<SdoMessage> {
    ...
    if (0x601..=0x67F).contains(&id) {
        ...
        if cs == 0x40 {
            ...
            && let Some((index, subindex)) = get_idx(payload)
            && payload.len() >= 8
        {
...
mod segmented_tests {
    ...
    fn test_sdo_segmented_download() {
        ...
        let payload = [0x11, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11];
